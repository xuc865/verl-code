#!/usr/bin/env bash
# CodeRL+-aligned GRPO from SFT ckpt — same recipe as mt8_v4 (see
# logs/grpo_coderl_qwen25_7b_mt8_v4.nohup.log), but:
#   - model = apps_mt8 SFT global_step_242
#   - 6×GPU 2–7 (train_batch must be divisible by n_gpus)
#
# Usage (train host):
#   bash /mnt/z4/solariewang/verl-swe/scripts/launch_grpo_coderl_sft_mt8.sh
#   FOREGROUND=1 bash .../launch_grpo_coderl_sft_mt8.sh
...
# ---- match successful mt8_v4 / one.sh offline train-node env (NOT HF_HUB_OFFLINE) ----
...
# Mild allocator fragmentation help; no throughput cost
export PYTORCH_CUDA_ALLOC_CONF=${PYTORCH_CUDA_ALLOC_CONF:-expandable_segments:True}
# Block accidental SwanLab cloud (same as one.sh)
_np="${NO_PROXY:-}"
...
# Same key as mt8_v4 / one.sh (file on train root). Local mode skips cloud login
# in verl/utils/tracking.py; key is kept for later DEV-side swanlab sync.
...
# prepare + run_swebench.sh read these
...
if (( ${#_GPU_ARR[@]} != N_GPUS )); then
  ...
  echo "  tip: unset CUDA_VISIBLE_DEVICES N_GPUS TRAIN_BATCH_SIZE  # clear stale 4-GPU env" >&2
  echo "  or:  CUDA_VISIBLE_DEVICES=2,3,4,5,6,7 N_GPUS=6 TRAIN_BATCH_SIZE=6 bash $0" >&2
  exit 1
fi
...
# agent path: actor_rollout_ref.rollout.n=1 → normalized_mini = PPO_MINI_BSZ // N_GPUS
...
# nohup child must inherit the resolved devices (not a stale parent export alone)
...
echo "CodeRL+ aligned GRPO (from SFT) | model=$MODEL_PATH | bench=apps_train_coderl"
...
# Same hydra overrides as mt8_v4; ENGINE 'vllm' is shifted inside run_swebench.sh
