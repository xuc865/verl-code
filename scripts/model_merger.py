    ...
    def get_transformers_auto_model_class(self):
        if "ForTokenClassification" in self.model_config.architectures[0]:
            ...
            if AutoModelForVision2Seq is None:
                raise ImportError(
                    "Need AutoModelForVision2Seq or AutoModelForImageTextToText "
                    f"for architecture {self.model_config.architectures[0]}"
                )
...
class FSDPModelMerger(BaseModelMerger):
    def _get_world_size(self) -> int:
        ...
        raise FileNotFoundError(
            f"Could not determine world size. No file matching 'model_world_size_*_rank_0.pt' found in {self.config.local_dir}"
        )
