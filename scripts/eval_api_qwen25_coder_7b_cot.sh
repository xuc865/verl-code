#!/usr/bin/env bash
# Qwen2.5-Coder-7B-Instruct — CoT baseline: multi-attempt freeform + CoT prompt.
#
# Order:
#   1) non-APPS queue
#   2) APPS (last)
#
# Protocol:
#   max_turns=12, test_feedback_mode=exec, enable_thinking
#   prompt_mode=freeform + encourage_cot (no ReAct XML)
#   METHOD=qwen25_coder_7b_mt12_cot_freeform
#
# Usage (training host):
#   cd /mnt/z4/solariewang/verl-swe
#   nohup bash scripts/eval_api_qwen25_coder_7b_cot.sh \
#     >> logs/eval_api_qwen25_coder_7b_mt12_cot_freeform_suite.nohup.log 2>&1 &
...
export METHOD=${METHOD:-qwen25_coder_7b_mt12_cot_freeform}
...
export ENCOURAGE_COT=${ENCOURAGE_COT:-1}
...
export WAIT_EVAL_PATTERN=${WAIT_EVAL_PATTERN:-python3.*eval_api_baseline.py.*qwen25_coder_7b_mt12_cot_freeform}

_ts() { date '+%Y-%m-%d %H:%M:%S'; }
mkdir -p "$LOG_DIR"
cd "$REPO_ROOT"

echo "[$(_ts)] ========== Qwen2.5-Coder-7B CoT (mt12 freeform) =========="
echo "  API=$API_BASE MODEL=$MODEL METHOD=$METHOD"
echo "  protocol=exec max_turns=12 thinking=ON prompt_mode=$PROMPT_MODE encourage_cot=$ENCOURAGE_COT"
echo "  order: non-APPS queue -> APPS (last)  workers=$EVAL_WORKERS"
...
echo "[$(_ts)] ========== CoT DONE (non-APPS then APPS) =========="
