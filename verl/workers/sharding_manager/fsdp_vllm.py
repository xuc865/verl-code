...
class FSDPVLLMShardingManager(BaseShardingManager):
    ...
    @staticmethod
    def _vllm_tp_device_group():
        """Resolve vLLM TP process group across API generations.

        - vLLM <=0.6.3: get_tensor_model_parallel_group() returns ProcessGroup
        - older 0.7–0.11: get_tensor_model_parallel_group().device_group
        - recent vLLM: get_tensor_model_parallel_group removed; use get_tp_group()
        """
    ...
    def preprocess_data(self, data: DataProto) -> DataProto:
        ...
        group = self._vllm_tp_device_group()
    ...
    def update_params(self, updated_params, peft_config=None):
        ...
        if peft_config:
            from verl.utils.vllm_utils import _VLLM_LORA_AVAILABLE

            if not _VLLM_LORA_AVAILABLE:
                raise ImportError(
                    "PEFT/LoRA sync requested but vLLM LoRA APIs are unavailable "
                    "(missing vllm.lora.models). Use full-finetune (lora_rank=0) or "
                    "install a compatible vLLM."
                )
            if self.base_sync_done:
