...
try:
    from vllm.config import LoRAConfig  # noqa: F401
...
try:
    ...
    LoRARequest = None  # type: ignore[misc, assignment]
    ...
    def generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        if self.lora_kwargs:
            if LoRARequest is None:
                raise ImportError(
                    "vllm.lora.request.LoRARequest is unavailable but lora_kwargs is set; "
                    "install a vLLM build with LoRA support or disable LoRA (lora_rank=0)."
                )
