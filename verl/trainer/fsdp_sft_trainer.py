...
# Qwen3.5 / VL: prefer Vision2Seq/ImageTextToText (ConditionalGeneration).
# CausalLM may map to a text-only class that expects top-level vocab_size/hidden_size.
try:
...
class FSDPSFTTrainer:
    ...
    def _build_model_optimizer(self):
        ...
        # Match fsdp_workers: multimodal configs (e.g. Qwen3.5) need Vision2Seq / ImageTextToText.
    ...
    def _compute_loss_and_backward(self, batch, do_backward=True):
        ...
        with context, torch.autocast(device_type=self.device_name, dtype=torch.bfloat16):
            if not use_sp:
                ...
                # Flatten the tokens
                # Qwen3.5 / VL configs keep vocab_size on text_config, not the top-level config.
                ...
                if vocab_size is None:
                    raise ValueError(f"Cannot resolve vocab_size from config type {type(self.model.config)}")
    ...
    def _list_local_checkpoints(self):
        """Return (step, path) for global_step_* dirs under default_local_dir, sorted by step ascending."""
    ...
    def _prune_old_checkpoints(self):
        """Keep at most max_ckpt_to_keep newest global_step_* dirs; delete older ones (rank0 only)."""
        max_keep = int(self.config.trainer.get("max_ckpt_to_keep", 0) or 0)
        if max_keep <= 0:
            return
        ckpts = self._list_local_checkpoints()
        if len(ckpts) <= max_keep:
            return
        for step, path in ckpts[: len(ckpts) - max_keep]:
            print(f"Pruning old SFT checkpoint (max_ckpt_to_keep={max_keep}): {path}")
            shutil.rmtree(path, ignore_errors=True)

    def _copy_hf_sidecars(self, ckpt_path: str):
        """Copy processor / generation configs from the base HF dir into a saved ckpt."""
        ...
        for name in (
            ...
            dst_f = os.path.join(ckpt_path, name)
            if os.path.isfile(src_f) and not os.path.isfile(dst_f):
                shutil.copy2(src_f, dst_f)
                print(f"Copied HF sidecar into ckpt: {name}")
    ...
    def save_checkpoint(self, step):
        ...
        # Copy to HDFS if configured; prune local slots after the new ckpt is written.
        if self.device_mesh.get_rank() == 0:
            # Multimodal bases (e.g. Qwen3.5) need processor sidecars for vLLM/HF reload.
            self._copy_hf_sidecars(path)
    ...
    def fit(self):
        ...
        for epoch in range(self.config.trainer.total_epochs):
            ...
            for data in tqdm(
                ...
                # Periodic checkpoint (same semantics as PPO trainer.save_freq: every N steps)
                ...
                if global_step >= self.total_training_steps:
                    ...
                    # Save final checkpoint (skip if just saved on this step)
                    if not (save_freq > 0 and global_step % save_freq == 0):
                        self.save_checkpoint(step=global_step)
                    return
            ...
            # Always save at end of each epoch. Skip if this step already hit save_freq.
            if not (save_freq > 0 and global_step % save_freq == 0):
                self.save_checkpoint(step=global_step)
