...
# LoRA helpers are only needed when actor_rollout_ref.model.lora_rank > 0.
# Newer vLLM builds may move/remove ``vllm.lora.models``; keep GRPO (no-LoRA)
# trainable by making these imports optional.
try:
    ...
    LoRAModel = None  # type: ignore[misc, assignment]
    LoRARequest = object  # type: ignore[misc, assignment]
    get_adapter_absolute_path = None  # type: ignore[assignment]
    LRUCacheWorkerLoRAManager = None  # type: ignore[misc, assignment]
...
class VLLMHijack():
    ...
    def hijack():
        if not _VLLM_LORA_AVAILABLE:
            # Standard GRPO has lora_rank=0; skip LoRA worker patch on incompatible vLLM.
            return
