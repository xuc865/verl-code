...
class Tracking:
    ...
    def __init__(self, project_name, experiment_name, default_backend: Union[str, List[str]] = "console", config=None):
        ...
        if "swanlab" in default_backend:
            ...
            # Train nodes often have no outbound net. Local/offline modes only need
            # disk logs; calling login() would hit the cloud and raise AuthenticationError.
            _local_modes = ("local", "offline", "disabled")
            if SWANLAB_API_KEY and SWANLAB_MODE not in _local_modes:
                swanlab.login(SWANLAB_API_KEY)  # NOTE: previous login information will be overwritten
            elif SWANLAB_MODE in _local_modes:
                # Ensure workers that missed env still do not attempt cloud auth.
                print(f"[tracking] swanlab mode={SWANLAB_MODE} logdir={SWANLAB_LOG_DIR} (skip cloud login)")

            if config is None:
                config = {}  # make sure config is not None, otherwise **config will raise error
            # Optional resume into an existing SwanLab run (same cloud/local run id).
            # Local mode still creates a new run-* dir, but keeps the same run id so
            # `swanlab sync ... -i <id>` appends to the previous cloud curve.
            swanlab_kwargs = {}
            SWANLAB_RUN_ID = os.environ.get("SWANLAB_RUN_ID") or os.environ.get("SWANLAB_ID")
            SWANLAB_RESUME = os.environ.get("SWANLAB_RESUME")
            if SWANLAB_RUN_ID:
                swanlab_kwargs["id"] = SWANLAB_RUN_ID
                # default allow: resume if cloud/local knows the id, else create new
                swanlab_kwargs["resume"] = SWANLAB_RESUME or "allow"
                print(
                    f"[tracking] swanlab resume id={SWANLAB_RUN_ID} "
                    f"resume={swanlab_kwargs['resume']} mode={SWANLAB_MODE}"
                )
            swanlab.init(
